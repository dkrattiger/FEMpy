...
class QuadPlaneStrain(element2D):
    ...
    def nodecoords(self):
        ...
        for i in range(0, mrl):
            for j in range(0, mrl):
                ...
                #  emat[i*mrl+j, :] = index_s[(i*n):(i*n+n+1),
                #                             (j*n):(j*n+n+1)].flatten('F')
        ...
        #  elements = []
        #  for i in range(0, np.shape(materialIndex)[0]):
        #      xEle = coordinates[emat[i, :], 0]
        #      yEle = coordinates[emat[i, :], 1]
